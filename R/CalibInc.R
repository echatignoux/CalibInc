...
predict_gam_fixed<-function(mod,newdata=NULL,type="lpmatrix"){
  ...
  Xs<-lapply(1:length(sre),function(i)
    ...
    do.call("cbind",.)
  drop_sm<-which((attr(stats::terms(mod),"variables")%>%as.character)[-c(1,2,attr(stats::terms(mod), "offset")+1)] %in% st)
  drop_sm<-paste0(".~.-",paste0(attr(stats::terms(stats::formula(mod)),"term.labels")[drop_sm],collapse="-"))

  pred<-newdata
  nst<-setdiff(all.vars(stats::formula(mod)),st)
  for (v in setdiff(nst,names(pred)))  pred[,v]<-1
  Xf<-stats::model.matrix(stats::update(stats::formula(mod),drop_sm),pred%>%data.frame)
  Xp<-cbind(Xf,Xs)
...
CalibInc<-function (mod, pred = NULL, weight = 1, aggregate = NULL, keep.Vp = FALSE)
{
  ...
  vars <- all.vars(stats::formula(mod))
  nb.can_ <- vars[attr(stats::terms(mod), "offset")]
  if (class(mod)[1] == "gam" & !is.null(mod_))
    if (!is.null(attr(stats::terms(mod), "offset")))
      nb.can_ <- as.character(attr(stats::terms(mod),"variables")[[attr(stats::terms(mod), "offset")+1]])[2]
  nb.bma_ <- vars[1]
  covars <- as.character(sapply(attr(stats::terms(stats::formula(mod)), "term.labels"),
                   function(tl) all.vars(stats::as.formula(paste0("~", tl)))[1]))
  if (class(mod)[1] == "glmmPQL")
  ...
  if (class(mod)[1] != "gam") Xp <- stats::model.matrix(lme4::nobars(formula(mod)), pred)
  ...
  if (length(unique(pred[,re][[1]]))>1)
    Xdep <- stats::model.matrix(as.formula(paste0("~-1+", re)),
                         data = pred %>%
                           dplyr::mutate_at(re, funs(f))) %>% Matrix::Matrix(.,sparse = T)
  else   Xdep <- stats::model.matrix(as.formula(~1),
                              data = pred) %>% Matrix::Matrix(.,sparse = T)
  Xag <- pred %>% dplyr::mutate_at(all.vars(ag), funs(f))
  Xag <- stats::model.matrix(ag, data = droplevels(Xag))
