...
ggLegMap<-function(legend=list(breaks=NULL,density=F,title = NULL),p,color,limits){
  ...
  if (is.null(limits)) {
    ...
    dt=tibble(x=1,y=1:length(color))
    bLeg<-(breaks-min(data$fill))/diff(range(data$fill))*(length(color))+.5
    bLeg<-tibble(x=0.5,xend=0.75,id=1)%>%
      left_join(tibble(y=bLeg,yend=bLeg,id=1))
  }
  else {
  ...
  if (!legDens){
    ...
    dt <- tibble(x=dt$x,y=dt$y/max(dt$y))
    dt <- tibble(x=dt$x,xmin=dt$x-diff(dt$x)[1]/2,xmax=dt$x+diff(dt$x)[1]/2,y=dt$y)
    if (!is.null(limits)){
      if (min(dt$x)> limits[1])
        dt<-rbind(dt,
                  tibble(x=seq(limits[1],min(dt$x),by=diff(dt$x)[1]))%>%
                    mutate(xmin=x-diff(dt$x)[1]/2,xmax=x+diff(dt$x)[1]/2,y=0),
                  tibble(x=limits[1],xmax=limits[1],xmin=limits[1]-min(diff(bLeg$breaks))/2,y=0))
      if (max(dt$x)< limits[2])
        dt<-rbind(dt,
                  tibble(x=seq(max(dt$x),limits[2],by=diff(dt$x)[1]))%>%
                    mutate(xmin=x-diff(dt$x)[1]/2,xmax=x+diff(dt$x)[1]/2,y=0),
                  tibble(x=limits[2],xmin=limits[2],xmax=limits[2]+min(diff(bLeg$breaks))/2,y=0))
    }
...
##' @param map Map shape (class "polygg", "SpatialPolygons" or "SpatialPolygonsDataFrame" ; set to ShapDep if NULL)
...
ggMap <-
  function (var, data = NULL, map = NULL, limits = NULL, breaks = NULL,
    ...
    ## Mef des donnees
    if (is.null(map))
      map <- CalibInc::ShapDep
    if (class(map)[1] %in% c("SpatialPolygons","SpatialPolygonsDataFrame"))
